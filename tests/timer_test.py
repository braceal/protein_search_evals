...
def test_timer_records_elapsed_time_and_tags(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with Timer('embedding', 2) as timer:
...
def test_time_logger_round_trip(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    logger = TimeLogger()
    ...
    assert logger.parse_logs(log_path) == [expected]
