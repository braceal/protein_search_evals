...
class HDF5TokenEmbeddings:
    ...
    def _append_batch_to_hdf5(self) -> None:
        # Get the batch size, embeddings, and embedding dimension
        batch_size = len(self.buffer)
        batch = self.buffer.embeddings
        embedding_dim = self.buffer.embedding_dim
        lengths = np.array(self.buffer.seq_lengths, dtype=np.int32)
    ...
    def flush(self) -> None:
        ...
        if len(self.buffer):
            # Append the buffer to the HDF5 file
